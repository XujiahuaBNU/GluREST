...
def ica_aroma_denoise(fslDir, inFile, mask, dim, TR, mc, denType, affmat, warp):

    import os

    mask_edge = '/scr/sambesi1/workspace/Projects/GluREST/denoise/mask_edge.nii.gz'
    mask_csf = '/scr/sambesi1/workspace/Projects/GluREST/denoise/mask_csf.nii.gz'
    mask_out = '/scr/sambesi1/workspace/Projects/GluREST/denoise/mask_out.nii.gz'
    ...
    def feature_spatial(fslDir, tempDir, melIC):
    ...
    melDirIn = ''
    runICA(fslDir, inFile, outDir, melDirIn, mask, dim, TR)
    ...
    edgeFract, csfFract = feature_spatial(fslDir, outDir, melIC_MNI)
    ...
    if denType is 'nonaggr':
        denoised = os.path.join(os.getcwd(), 'denoised_func_data_nonaggr.nii.gz')
    elif denType is 'aggr':
        denoised = os.path.join(os.getcwd(), 'denoised_func_data_aggr.nii.gz')
...
def ica_aroma_workflow():
    ...
    inputnode  = Node(util.IdentityInterface(fields=['fslDir', 'inFile', 'mask', 'dim', 'TR', 'mc', 'denType', 'affmat', 'warp']),
                     name = 'inputnode')

    outputnode = Node(util.IdentityInterface(fields=['denoised']),
                     name = 'outputnode')

    aroma = Node(util.Function(input_names   = ['fslDir', 'inFile',  'mask','dim', 'TR', 'mc', 'denType', 'affmat', 'warp'],
